...
dt_03[,"ID" := paste0(dt_03$qtype, dt_03$ser_no)]
dt_07[,"ID":= paste0(dt_07$qtype, dt_07$ser_no)]
survive <- intersect(dt_03$ID, dt_07$ID)

ss<- dt_03[ID %in% survive, c("ID","0_age", "0_edu")]
setnames(ss, c("ID", "0_age", "0_edu"), c("ID", "03age", "03edu"))
dt_07<- ss[dt_07, on="ID"]

# 0_ 為伴侶資料
# 一樣沒更新新結婚的
dt_07[, "0_relation":= 2,]
dt_07[, "0_gender":=  ifelse(dt_07$cvr1 == 1, 2 , 1 )]
dt_07[, "0_gender":=  ifelse(dt_07$a1_1 %in% c(2,3,9), 8 , dt_07$"0_gender" )]
dt_07[, "0_age":= dt_07$"03age"+4]
...
cols<- c(unlist( lapply(paste0("^", 0:30,"_"), function(x){grep(x, names(dt_07), value = T)})))
class(cols)
...
dt_07_reshape <- reshape(tt, varying =x , direction = "long", timevar = "ID_i", idvar = c("ser_no", "survey_year", "qtype","adl_who_help", 'iadl_who_help') , v.names = c("relationship","gender","age","edu","work","marstat","livstat","livwhere","meet","tele"))
