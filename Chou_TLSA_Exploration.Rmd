...
```{r}
subject.dt <- as.data.table(read.csv("LTC_TSLA-main/main_subject_0807.csv"))
fmlymmb.dt <- as.data.table(read.csv("LTC_TSLA-main/family_member_0815.csv"))
```
...
### Preprocess
```{r}
fmlymmb.dt$gender = ifelse(fmlymmb.dt$gender == 1, 1, 0)
...
fmlymmb.dt$fmNeedADL = ifelse(fmlymmb.dt$adl_who_help !=88, 1, 0)
fmlymmb.dt$fmNeedIADL = ifelse(fmlymmb.dt$iadl_who_help !=88, 1, 0)
# Define degree of intimacy
```

```{r}
non_married.obs <- subject.dt[marstat == 0]$qser_no
fmlymmb.dt.nmar <- fmlymmb.dt[qser_no %in% non_married.obs]
fmlymmb.dt$ObsPartnerY <- ifelse(fmlymmb.dt$qser_no %in% non_married.obs, 0, 1)
```
...
need.reg3 <- feols(primaryADL ~ gender + age + age**2 + education | survey_year, fm.with.need)
need.reg4 <- feols(primaryADL ~ gender + age + age**2 + education + working | survey_year, fm.with.need)
need.reg5 <- feols(primaryADL ~ gender + age + age**2 + education + working + unmarried | survey_year, fm.with.need)
need.reg6 <- feols(primaryADL ~ gender + age + age**2 + education + working + unmarried + SubSameSex | survey_year, fm.with.need)
need.reg7 <- feols(primaryADL ~ gender + age + age**2 + education + working + unmarried + SubSameSex + partner | survey_year, fm.with.need)
need.reg8 <- feols(primaryADL ~ gender + age + age**2 + education + working + unmarried + SubSameSex + partner + livTogether | survey_year, fm.with.need)
...
simple.tree <- rpart(primaryADL ~ gender + age + age**2 + 
                       education + partner + working + SubSameSex + 
...
### PCA Clustering
```{r}
data.pca <- fm.with.need[,c("gender", "age", "education", "working", "unmarried", "SubSameSex", "partner", "livTogether", "primaryADL")]
...
odd.obs <- unique(fmlymmb.dt.nmar[relationship==2]$qser_no)
...
need.reg5 <- feols(primaryADL ~ gender + age + age**2 + education + working + unmarried | survey_year, fm.with.need)
need.reg6 <- feols(primaryADL ~ gender + age + age**2 + education + working + unmarried + SubSameSex | survey_year, fm.with.need)
need.reg7 <- feols(primaryADL ~ gender + age + age**2 + education + working + unmarried + SubSameSex + livTogether | survey_year, fm.with.need)
etable(need.reg1, need.reg2, need.reg3, need.reg4, need.reg5, need.reg6, need.reg7)
```
...
### PCA
```{r}
data.pca <- fm.with.need[,c("gender", "age", "education", "working", 
