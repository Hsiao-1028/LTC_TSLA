...
write.table(fam, file = "undetected_fam_v7.csv", sep = ",") 
...
```{r}
# matching 
fam<- read.csv("/Users/hsiao/Desktop/Projects/HRS_Chao/LTC_TSLA/Main code/undetected_fam_v7.csv") # added spouse and iadl and children
...
# 如果只有一年的話，有重複也要挑出來
detect_1<- function(u,data,ID){
for (x in unique(data$ID)){
  if(length(data[ ID ==x &survey_year ==u ]$birthyear)>1 |length(data[ ID ==x &survey_year ==u ]$gender)>1 | length(data[ ID ==x &survey_year ==u ]$education)>1 ){ data[ID ==x, age_acc:= FALSE][ID ==x, gender_acc:= FALSE][ID ==x, education_acc:= FALSE] }
}}

re_detect_1<- function(u){
for (x in unique(remaining$k_group)){
  if(length(remaining[ k_group ==x &survey_year ==u ]$birthyear)>1 |length(remaining[ k_group ==x &survey_year ==u ]$gender)>1 | length(remaining[ k_group ==x &survey_year ==u ]$education)>1){ remaining[k_group ==x, k_age_acc:= FALSE][k_group ==x, k_gender_acc:= FALSE][k_group ==x, k_education_acc:= FALSE]}
  else if ( length( remaining[ k_group ==x ]$survey_year) ==1){ remaining[k_group ==x, k_age_acc:= TRUE][k_group ==x, k_gender_acc:= TRUE][k_group ==x, k_education_acc:= TRUE]}
  }}
...
```{r}
# 媳婦
detect_dil<- function(x,y,z,u,data =data ,ID ="ID"){
...
re_detect_dil<- function(x,y,z,u){
  remaining[, k_age_acc :=NA][, k_education_acc :=NA][, k_gender_acc :=NA]
...
original <- copy(data)
detect_dil(1996,1999,2003,2007, data, "ID")


# for (x in unique(data$qser_no)){
#  if(length(data[ qser_no ==x &survey_year ==1993 ]$birthyear)>0 |length(data[ qser_no ==x &survey_year ==1993 ]$gender)>0 | length(data[ qser_no ==x &survey_year ==1993 ]$education)>0 ){ data[qser_no ==x, age_acc:= FALSE][qser_no ==x, gender_acc:= FALSE][qser_no ==x, education_acc:= FALSE] 
#  }
#  else{next}
#   }
...
rescue_1[, unif := paste0(survey_year, qser_no, relationship)]

duplcaited_unif<- rescue_1[ duplicated(unif)]$qser_no
unsolve<- append(unsolve, duplcaited_unif)
solve_1<- setdiff( problems, unsolve) #for updating
rescue_1 <- data[qser_no %in% solve_1][, problem :=1] #for updating

remaining<- copy(data[qser_no %in% unsolve])
re_detect_dil(1996,1999,2003,2007) # is a mess的人
...
original_grouping<- daughters_in_law_all[ qtype =="B" & survey_year != 1993]
original_grouping[, unif := ifelse(problem ==0, paste0(survey_year, qser_no, relationship), paste0(survey_year, k_group) )]
original_grouping[, ser_no := as.numeric(as.character(str_sub(qser_no, 2)))]

jest<- original_grouping
jest[, d_dil := ifelse(problem !=2, str_sub(ID, -1), str_sub(k_group,-1))][, n_dil := length(unique(d_dil)), by= qser_no]
jest[, dd_dil := paste(qser_no, d_dil, sep = "_")]
jjest<- jest[ !duplicated(jest$dd_dil)][ ,c("qser_no", "birthyear","education", "d_dil","dd_dil","n_dil","gender")]

dil_1993 <- daughters_in_law_all[ qtype =="B" & survey_year ==1993 ]
dil_1993<- dil_1993[, ser_no := as.numeric(as.character(str_sub(qser_no, 2)))]
kest<- dil_1993


kest[, match:=0][, unif_dil := paste0(qser_no,"_", .I)]
#sub(" xxx.*", "", x) 

for( i in unique(kest$unif_dil)){
  for( j in unique(jjest[ qser_no == sub("_.*","", i)]$d_dil))
    if( kest[unif_dil == i ]$match ==0){
 kest[unif_dil ==i , match:= ifelse( abs(birthyear - jjest[ qser_no ==sub("_.*","", i) & d_dil ==j]$birthyear)<4 
                                    & abs(gender - jjest[ qser_no ==sub("_.*","", i) & d_dil ==j]$gender)<1 
                                    & abs(education - jjest[ qser_no ==sub("_.*","", i) & d_dil ==j]$education)<7
                                    , 90 +as.numeric(as.character(j)),0)]
    }
  else{next}
}

kest[, match:= ifelse(match ==0, 96, match)]
kest[, ID := paste(qser_no, match, sep = "_")]
j<-kest[duplicated(kest$ID)]$qser_no # 不是之後年份沒有，就是match不到(數字有差) # 69人

kest[, relationship:= match]
table(kest$relationship) # 大概還是有一半的人沒有match上

kest[qser_no %in% unique(rescue_2$qser_no) & match!=96, relationship:= 96]
kest[qser_no %in% unique(rescue_2$qser_no) & match!=96, problem:=2][qser_no %in% unique(rescue_2$qser_no) & match==96, problem:=0]
kest[qser_no %in% unique(rescue_2$qser_no) & match!=96, k_group:=paste0(qser_no, "_", match-90)][qser_no %in% unique(rescue_2$qser_no) & match==96, k_group:=NA]

#setdiff(colnames(daughters_in_law_all),colnames(kest))

kest[, c("match","unif_dil"):=NULL]

data<- rbind(daughters_in_law_all[ survey_year!=1993], kest)
...
# try[ qser_no =="B2344" , problem:=2]
# 
# check<- percise[qser_no =="B2344"]
# check<- rescue_1[duplicated(rescue_1$unif)]
# data<- rescue_1[qser_no %in% c("B4038","B2602","B3320")]
# for (i in unique(data$qser_no)){
#   prepare_kmeans(i, data)}
# for (i in unique(data$qser_no)){
#   get_kmeans(i, data)}
...
# s<- as.data.table(as.matrix(s))
# unsolve<- unique(data[ID %in% unique(s$V1)]$qser_no)
# solve_1<- setdiff( problems, unsolve) 
# rescue_1 <- data[qser_no %in% solve_1][, problem :=1] 
# remaining<- copy(data[qser_no %in% unsolve])
# re_detect(1993,1996,1999,2003,2007)
# rescue_2 <- remaining[k_age_acc == TRUE & k_education_acc ==TRUE & k_gender_acc ==TRUE][ ,problem:=2]


# try <- rbind(try[ qser_no != "B4038"], rescue_2, fill = TRUE)
dil_all <- try
dil<- try[, c("qser_no","survey_year", "ID","k_group", "problem","relationship","gender","age","edu","education","work","marstat","children","livstat","livwhere","meet","tele","adl_who_help", "iadl_who_help")]
# dil<- dil[ !( qser_no =="B3320" & survey_year =="1996" & age ==27 )]


dil[, unif:= ifelse(problem!=0,paste0(survey_year, k_group),paste0(survey_year, qser_no, relationship))]
nrow(dil) == length(unique(dil$unif)) # 7132
p<- dil[duplicated(dil$unif)]

# 最後處理duplicated的問題
p<- dil[ qser_no %in% unique(p$qser_no)]
...
unsolve<- unique(data[ID %in% unique(s$V1)]$qser_no)
solve_1<- setdiff( problems, unsolve)
rescue_1_p <- data[qser_no %in% solve_1][, problem :=1]
remaining<- copy(data[qser_no %in% unsolve])
re_detect(1993,1996,1999,2003,2007)
rescue_2_p <- remaining[k_age_acc == TRUE & k_education_acc ==TRUE & k_gender_acc ==TRUE][ ,problem:=2]

dil<- dil[ unif %nin% unique(p$unif)]

# dil[ qser_no == "B2216", problem:=2]

 #write.csv(dil, "dil.csv")
# remaining work : match back !
# jj<- kest[relationship==96]$qser_no
# table(kest$relationship) #match 不到？
# dil<- rbind(original_grouping, kest)
# sum(daughters_in_law[survey_year ==1993]$qser_no %in% daughters_in_law[survey_year!=1993]$qser_no) # 397個之後沒有了 1042個還有出現

# setdiff(colnames(data), colnames(test))
# data[, c(setdiff(colnames(data), colnames(test))):=NULL]
# 
# 
# copycc<- copy(dil)
# 
# percise[, unif := paste0(survey_year, qser_no, relationship)]
# rescue_1[, unif := paste0(survey_year, k_group)]
# rescue_2[, unif:= paste0(survey_year, k_group)]
# 
# nrow(percise) == length(unique(percise$unif))
# nrow(rescue_1) == length(unique(rescue_1$unif))
# nrow(rescue_2) == length(unique(rescue_2$unif))
# 
...
j<-percise[duplicated(percise$unif)]


remaining$y_q_ser_no <- paste0(remaining$survey_year, remaining$qser_no)
rescue_2$y_q_ser_no <- paste0(rescue_2$survey_year, rescue_2$qser_no)
library(Hmisc)
trash<-remaining[y_q_ser_no %nin% unique(rescue_2$y_q_ser_no)]
...
remaining<- copy(data[qser_no %in% unsolve]) # A1344
re_detect(1993,1996,1999,2003,2007) # is a mess的人
...
data <- test[relationship %in% c(50:62) & age >=7]
...
next_gen_all[, unif:= ifelse( problem %in% c(0,1), paste0(survey_year, qser_no, relationship), paste0(survey_year, k_group))]

sum(duplicated(next_gen_all$unif))
...
try <- rbind(percise, rescue_1, rescue_2, fill=TRUE)
youngs_all <- try
youngs<- try[, c("qser_no","survey_year", "ID","k_group", "problem","relationship","gender","age","edu","education","work","marstat","children","livstat","livwhere","meet","tele","adl_who_help", "iadl_who_help")]
...
youngs[, unif:= ifelse( problem %in% c(0,1), paste0(survey_year, qser_no, relationship), paste0(survey_year, k_group))]

sum(duplicated(youngs$unif))
...
full_spouses <- test[relationship %in% c(2,3)][, c("qtype","ser_no"):= NULL][, problem:= 0]
fam_re <- rbind(full_spouses, elders,same_gen, son_1, son_2, daughters_1, daughters_2, dil, next_gen,youngs, others, fill = TRUE)


fam_re[ edu == 88 & problem ==0, problem:= 3][, youngs := ifelse(is.na(youngs), 0, youngs)]

fam_re[, kk_group:= ifelse( !is.na(fam_re$k_group) & fam_re$relationship %in% c(2:4), paste0(fam_re$qser_no,"_", "spouses","_",sub(".*_", "", k_group)), NA)][, kk_group:= ifelse( !is.na(fam_re$k_group) & fam_re$relationship %in% c(5:19), paste0(fam_re$qser_no,"_", "elders","_" ,sub(".*_", "", k_group)), fam_re$kk_group)][, kk_group:= ifelse( !is.na(fam_re$k_group) & fam_re$relationship %in% c(20:29), paste0(fam_re$qser_no,"_", "same_generation","_", sub(".*_", "", k_group)), fam_re$kk_group)][, kk_group:= ifelse( !is.na(fam_re$k_group) & fam_re$relationship %in% c(30:34), paste0(fam_re$qser_no,"_", "first_four_sons", "_",sub(".*_", "", k_group)), fam_re$kk_group)][, kk_group:= ifelse( !is.na(fam_re$k_group) & fam_re$relationship %in% c(35:39), paste0(fam_re$qser_no,"_" ,"fifth_sons_and_after","_" ,sub(".*_", "", k_group)), fam_re$kk_group)][, kk_group:= ifelse( !is.na(fam_re$k_group) & fam_re$relationship %in% c(40:44), paste0(fam_re$qser_no,"_","first_four_daughters","_" ,sub(".*_", "", k_group)), fam_re$kk_group)][, kk_group:= ifelse( !is.na(fam_re$k_group) & fam_re$relationship %in% c(45:49), paste0(fam_re$qser_no,"_", "fifth_daughters_and_after","_",sub(".*_", "", k_group)), fam_re$kk_group)][, kk_group:= ifelse( !is.na(fam_re$k_group) & fam_re$relationship %in% c(50:62), paste0(fam_re$qser_no,"_", "grand_sons","_", sub(".*_", "", k_group)), fam_re$kk_group)][, kk_group:= ifelse( !is.na(fam_re$k_group) & fam_re$relationship %in% c(91:96), paste0(fam_re$qser_no,"_", "dauhters_in_law","_",sub(".*_", "", k_group)), fam_re$kk_group)][, kk_group:= ifelse( !is.na(fam_re$k_group) & fam_re$relationship %in% c(70:83, 97), paste0(fam_re$qser_no, "_","others","_",sub(".*_", "", k_group)), fam_re$kk_group)]

fam_re[, kk_y_group:= ifelse( problem %in% c(1,2) & youngs ==1, paste0(kk_group,"_", "young"), kk_group )]
fam_re[, kk_group := ifelse(youngs ==1, kk_y_group, kk_group)]
# [, kk_group:= ifelse( !is.na(fam_re$k_group) & youngs ==1, paste0(fam_re$kk_group, "_","youngs"), fam_re$kk_group)]



fam_re[, unif := ifelse(problem %in% c(0,1), paste(survey_year, qser_no, relationship, sep = "_"), paste(survey_year, kk_group, sep="_"))]
fam_re[, unif:= ifelse( problem ==3, paste(survey_year, qser_no, relationship, "young", sep =  "_"), unif)]
fam_re[, problem := ifelse(problem ==3, 0, problem)]


# check for Identity code unificated
sum(duplicated(fam_re$unif))
l<- fam_re[duplicated(fam_re$unif)]$unif


table(fam_re$problem)
...
fam_super_na<- as.data.table(fam_super_na)
```
...
sum(duplicated(fam_super_na$unif))
fam_super_na<- fam_super_na[ !( relationship ==2 & is.na(marstat))]

fam_super_na[, qser_no_told_relation := qser_no_relation]
fam_super_na[, c("kk_y_group", "ID","X","qser_no_relation"):= NULL]

# write.csv(fam_super_na, "family_member_1111.csv")

```

```{r}
# 改1993年的 main giver, if main giver == 96

t<- fam_super_na[ survey_year ==1993 & relationship %in% c(91:97)]
dil_both<- t[ adl_who_help ==40 & iadl_who_help ==40]

for( i in unique(dil_both$qser_no)){
   if( nrow(dil_both[qser_no == i] ) > 1){
     dil_both[ qser_no ==i, adl_who_help:=96]
     dil_both[ qser_no ==i, iadl_who_help:=96]
   }
  else{
   dil_both[ qser_no==i , adl_who_help:= relationship]
   dil_both[ qser_no==i , iadl_who_help:= relationship]
  }
}

dil_iadl<- t[ iadl_who_help ==40 & qser_no %nin% dil_both$qser_no]

for( i in unique(dil_iadl$qser_no)){
   if( nrow(dil_iadl[qser_no == i] ) > 1){
     dil_iadl[ qser_no ==i, iadl_who_help:=96]
   }
  else{
    dil_iadl[ qser_no==i , iadl_who_help:= relationship]
  }
}

dil_adl<- t[ adl_who_help ==40 & qser_no %nin% dil_both$qser_no]

for( i in unique(dil_adl$qser_no)){
   if( nrow(dil_adl[qser_no == i] ) > 1){
     dil_adl[ qser_no ==i, adl_who_help:=96]
   }
  else{
    dil_adl[ qser_no==i , adl_who_help:= relationship]
  }
}

dil_rematch <- rbind(dil_both, dil_adl, dil_iadl)
dil_rematch<- dil_rematch[ , c("qser_no", "adl_who_help","iadl_who_help")]
dil_rematch<- dil_rematch[ ! duplicated(dil_rematch$qser_no)]
# check<- fam_super_na[ survey_year ==1993 & adl_who_help ==40]


for ( i in unique(dil_rematch$qser_no)){
   fam_super_na[ survey_year ==1993 & qser_no ==i, iadl_who_help := dil_rematch[ qser_no ==i ]$iadl_who_help ]
   fam_super_na[ survey_year ==1993 & qser_no ==i, adl_who_help := dil_rematch[ qser_no ==i ]$adl_who_help ]
}

problem_1993_dil<- fam_super_na[ survey_year ==1993 & iadl_who_help ==40]$qser_no # 這些人是沒有回報有媳婦，但是iadl跑出媳婦的人

for ( i in problem_1993_dil){
   fam_super_na[ survey_year ==1993 & qser_no ==i, iadl_who_help :=96 ]
}


# j<- rbind(t[ qser_no %nin% unique(dil_rematch$qser_no)], dil_rematch)
# fam_super_na<- rbind(fam_super_na[survey_year !=1993 | relationship %nin% c(91:97)], j)


subject<- read.csv("/Users/hsiao/Desktop/Projects/HRS_Chao/LTC_TSLA/main_subject_1026.csv")
subject<- as.data.table(subject)

for ( i in unique(dil_rematch$qser_no)){
   subject[ survey_year ==1993 & qser_no ==i, iadl_who_help := dil_rematch[ qser_no ==i ]$iadl_who_help ]
   subject[ survey_year ==1993 & qser_no ==i, adl_who_help := dil_rematch[ qser_no ==i ]$adl_who_help ]
}

for ( i in problem_1993_dil){
   subject[ survey_year ==1993 & qser_no ==i, iadl_who_help :=96 ]
}

subject[ qser_no == "B1381" & survey_year ==1993, adl_who_help :=96]
subject[ qser_no == "B1381" & survey_year ==1993, iadl_who_help :=96]
subject[ qser_no == "B2969" & survey_year ==1993, iadl_who_help :=96]


subject<- subject[, X:= NULL]

# write.csv(fam_super_na, "family_member_1111.csv")
# write.csv(subject, "main_subject_1111.csv", row.names = FALSE)
...
#fam_super_na<- read.csv("/Users/hsiao/Desktop/Projects/HRS_Chao/LTC_TSLA/family_member_1111.csv")
#subject<- read.csv("/Users/hsiao/Desktop/Projects/HRS_Chao/LTC_TSLA/main_subject_1111.csv")



# Easiler to read 
setnames(fam_super_na, "gender","male")
fam_super_na$male <- ifelse(fam_super_na$male ==1,1,0)

fam_super_na<- as.data.table(fam_super_na)
subject<- as.data.table(subject)

subject[, y_qser_no := paste(survey_year, qser_no, sep ="_")]
fam_super_na[, y_qser_no := paste(survey_year, qser_no, sep ="_")]

nursing_house_id<- subject[dwell1 ==2]$y_qser_no
fam_super_na <- fam_super_na[ , subject_in_nursing_house := ifelse(y_qser_no %in% nursing_house_id , 1, 0)]
main_subject_residency <- subject[ ,c("y_qser_no", "crt1")]
fam_super_na<- main_subject_residency[ fam_super_na, on = "y_qser_no"][, X:=NULL]
setnames(fam_super_na, "crt1", "subject_residency")

fam_super_na[, having_child := ifelse(fam_super_na$children >0, 1, 0)]
setnames(fam_super_na, "children", "number_of_children")


# relationship
fam_super_na[, is.spouse:= ifelse( relationship %in% c(2,4), 1,0)]
fam_super_na[, is.elder := ifelse( relationship %in% c(5:19), 1,0)]
fam_super_na[, is.same_generation := ifelse( relationship %in% c(20:29), 1,0)]
fam_super_na[, is.son := ifelse( relationship %in% c(30:39), 1,0)]
fam_super_na[, is.daughter := ifelse( relationship %in% c(40:49), 1,0)]
fam_super_na[, is.grandson := ifelse( relationship %in% c(50:62), 1,0)]
fam_super_na[, is.daughter_in_law := ifelse( relationship %in% c(91:96), 1,0)]
fam_super_na[, is.other := ifelse( relationship %in% c(70:83, 97), 1,0)]

fam_super_na[, unif:= substr(unif, 6, nchar(unif))]

fam_super_na[problem ==2 & relationship %in% c(30:34), unif:= paste(qser_no, "son", sub(".*_", "", k_group), sep = "_")]
fam_super_na[problem ==2 & relationship %in% c(30:34), j := sub(".*_", "", k_group)]
invisible(for (i in unique(fam_super_na$qser_no)){
fam_super_na[ qser_no == i, sons:= max(j, na.rm = TRUE)]})
fam_super_na[problem ==2 & relationship %in% c(35:39), unif:= ifelse( is.na(sons), paste(qser_no, "son", 4+ as.numeric(sub(".*_", "", k_group)), sep = "_"), paste(qser_no, "son", as.numeric(sub(".*_", "", k_group)) + as.numeric(sons), sep = "_"))]

fam_super_na[problem ==2 & relationship %in% c(40:44), unif:= paste(qser_no, "daughter", sub(".*_", "", k_group), sep = "_")]
fam_super_na[problem ==2 & relationship %in% c(40:44), j := sub(".*_", "", k_group)]
invisible(for (i in unique(fam_super_na$qser_no)){
fam_super_na[problem ==2 & qser_no == i, sons:= max(j, na.rm = TRUE)]})
fam_super_na[problem ==2 & relationship %in% c(45:49), unif:= ifelse( is.na(sons), paste(qser_no, "daughter", 4+ as.numeric(sub(".*_", "", k_group)), sep = "_"), paste(qser_no, "daughter", as.numeric(sub(".*_", "", k_group)) + as.numeric(sons), sep = "_"))]

fam_super_na[youngs ==1 & relationship %in% c(30:49), unif := paste(unif, "young", sep = "_")][,c("sons", "j") := NULL]

# fam_super_na[, check:= paste0(survey_year, unif)]
setnames(fam_super_na, c("relationship","having_child"),c("told_relation","have_child"))
fam_super_na[, c("qser_no_told_relation","kk_group","X"):=NULL]

setcolorder(fam_super_na, c("unif","problem", "survey_year","told_relation", "male", "age", "birthyear", "edu","work", "marstat","have_child","number_of_children" ,"livstat","livwhere","meet","tele","y_qser_no","adl_who_help","iadl_who_help","subject_residency","subject_in_nursing_house","is.spouse","is.elder","is.same_generation","is.son","is.daughter","is.grandson","is.daughter_in_law","is.other","k_group","youngs","age_missing","qser_no"))

colnames(fam_super_na)
write.csv(fam_super_na, "family_member_1111.csv", row.names = FALSE)
write.csv(subject, "main_subject_1111.csv", row.names = FALSE)
