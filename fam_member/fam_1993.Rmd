...
dt_93[, "0_relation":= 2,]
dt_93[, "0_gender":=  ifelse(dt_93$'89_sex' == 1, 2 , 1 )][, '0_gender':= ifelse(dt_93$a1cores ==1, dt_93$'0_gender', 8)]
# 分居
dt_93[, "0_gender":=  ifelse(dt_93$a1awyspe == 1, ifelse(dt_93$`89_sex` ==1,2,1) , dt_93$`0_gender` )]

dt_93[, "0_age":= as.numeric(as.character(dt_93$'89_sage'))+3 ] # 要從1989年的回推
dt_93[, "0_edu":=  dt_93$`89edu`] # Invarance 

dt_93[, "0_work" := NA]
dt_93[, "0_work":= ifelse( as.numeric( as.character(dt_93$a1cores))==1, as.numeric(as.character(dt_93$a5c)), dt_93$'0_work')][, "0_work":= ifelse(dt_93$"0_work" ==2, 5,dt_93$"0_work" )][, "0_work":= ifelse(dt_93$"0_work" ==8, NA ,dt_93$"0_work" )] #不是一直有婚者我真處理不太了，那邊的coding都亂掉.. 

dt_93[, "0_marstat":= ifelse(dt_93$a1cores ==1, 1, 8) ] # 沒辦法處理新結婚的4人
...
dt_93[, "0_age" := ifelse(dt_93$a4acores %in% c(1,2),  82 - as.numeric(as.character(dt_93$a4byr)), dt_93$`0_age`)]
...
for(i in 2:9){setnames(dt_93, paste0( "bx62p0", i), paste0(i-1 ,"_work"))}
...
# for(i in 2:9){setnames(dt_93, paste0( "b5ap0", i), paste0(i-1 ,"_livstat"))} # 0 在codebook裏面是 88 


for(i in 10:16){setnames(dt_93, paste0( "relp", i), paste0(i-1 ,"_relation"))}
# for(i in :4){setnames(dt_93, paste0( "sexp0", i), paste0(i-1 ,"_gender"))} # 05 ! 
for(i in 10:16){setnames(dt_93, paste0( "sexp", i,"m"), paste0(i-1 ,"_gender"))} # 05 ! 

for(i in 10:16){setnames(dt_93, paste0( "bx2p", i), paste0(i-1 ,"_age"))}
for(i in 10:16){setnames(dt_93, paste0( "bx5p", i), paste0(i-1 ,"_edu"))}
for(i in 10:16){setnames(dt_93, paste0( "bx62p", i), paste0(i-1 ,"_work"))}
...
# for(i in 10:16){setnames(dt_93, paste0( "b5ap", i), paste0(i-1 ,"_livstat"))} # 0 應該是住在一起？？

# for ( i in 2:9){ dt_93[, paste0(i-1 , "_work"):= ifelse(paste0("bx61p0", i) %nin% c(8,9), 1, paste0("bx62p0", i)  )]}
...
# dt_93$`3_livstat` <- ifelse(dt_93$`3_livstat` !=0, 1, 0)
# dt_93$`4_livstat` <- ifelse(dt_93$`4_livstat` !=0, 1, 0)
# dt_93$`5_livstat` <- ifelse(dt_93$`5_livstat` !=0, 1, 0)
# dt_93$`6_livstat` <- ifelse(dt_93$`6_livstat` !=0, 1, 0)
# dt_93$`7_livstat` <- ifelse(dt_93$`7_livstat` !=0, 1, 0)
# dt_93$`8_livstat` <- ifelse(dt_93$`8_livstat` !=0, 1, 0)
# dt_93$`9_livstat` <- ifelse(dt_93$`9_livstat` !=0, 1, 0)
# dt_93$`10_livstat` <- ifelse(dt_93$`10_livstat` !=0, 1, 0)
# dt_93$`11_livstat` <- ifelse(dt_93$`11_livstat` !=0, 1, 0)
# dt_93$`12_livstat` <- ifelse(dt_93$`12_livstat` !=0, 1, 0)
# dt_93$`13_livstat` <- ifelse(dt_93$`13_livstat` !=0, 1, 0)
# dt_93$`14_livstat` <- ifelse(dt_93$`14_livstat` !=0, 1, 0)
# dt_93$`15_livstat` <- ifelse(dt_93$`15_livstat` !=0, 1, 0)
# dt_93$`16_livstat` <- ifelse(dt_93$`16_livstat` !=0, 1, 0)
...
for(i in 1:9){setnames(dt_93, paste0( "sexq0", i), paste0(i+15 ,"_gender"))} # 05 ! 

# for(i in 5:9){setnames(dt_93, paste0( "sexp0", i,"m"), paste0(i-1 ,"_gender"))} # 05 ! 
for(i in 1:9){setnames(dt_93, paste0( "by2q0", i), paste0(i+15 ,"_age"))}
for(i in 1:9){setnames(dt_93, paste0( "by5q0", i), paste0(i+15 ,"_edu"))}
for(i in 1:9){setnames(dt_93, paste0( "by62q0", i), paste0(i+15 ,"_work"))}
...
for(i in 10:15){setnames(dt_93, paste0( "sexq", i), paste0(i+15 ,"_gender"))} # 05 ! 

# for(i in 5:9){setnames(dt_93, paste0( "sexp0", i,"m"), paste0(i-1 ,"_gender"))} # 05 ! 
for(i in 10:15){setnames(dt_93, paste0( "by2q", i), paste0(i+15 ,"_age"))}
for(i in 10:15){setnames(dt_93, paste0( "by5q", i), paste0(i+15 ,"_edu"))}
for(i in 10:15){setnames(dt_93, paste0( "by62q", i), paste0(i+15 ,"_work"))}
...
for(i in 10:15){setnames(dt_93, paste0( "by9q", i), paste0(i+15 ,"_meet"))} # 你拜訪他 # 他拜訪你是by11q0_
...
# 其他交換照顧者
for(i in 1:9){setnames(dt_93, paste0( "relr0", i), paste0(i+30 ,"_relation"))}
for( i in 1:10){dt_93[, paste0(i+30,"_gender"):= NA ]}
for( i in 1:10){dt_93[, paste0(i+30,"_age"):= NA ]}
for( i in 1:10){dt_93[, paste0(i+30,"_edu"):= NA ]}
for( i in 1:10){dt_93[, paste0(i+30,"_work"):= NA ]}
for( i in 1:10){dt_93[, paste0(i+30,"_marstat"):= NA ]}
for( i in 1:10){dt_93[, paste0(i+30,"_children"):= NA ]}
for( i in 1:10){dt_93[, paste0(i+30,"_livstat"):= NA ]}
for( i in 1:10){dt_93[, paste0(i+30,"_livwhere"):= NA ]}
for( i in 1:10){dt_93[, paste0(i+30,"_meet"):= NA ]}
for( i in 1:10){dt_93[, paste0(i+30,"_tele"):= NA ]}
for(i in 1:9){setnames(dt_93, paste0( "b15r0", i), paste0(i+30 ,"_help_iadl"))}
for(i in 1:9){setnames(dt_93, paste0( "b16r0", i), paste0(i+30 ,"_help_adl"))}

setnames(dt_93, c('relr10', 'b15r10', 'b16r10'), c("40_relation", "40_help_iadl", "40_help_adl"))


cols<- c(unlist( lapply(paste0("^", 0:40,"_"), function(x){grep(x, names(dt_93), value = T)})))
# class(cols)
...
#first deal help ?? 重複??
liv<- c("_relation","_gender","_age","_edu","_work","_marstat","_children","_livstat","_livwhere","_meet","_tele", "_help_adl", "_help_iadl")

cols<- unlist(lapply(0:40, function(x){paste0(x,liv)}))
cols<- append(c("ser_no", "survey_year", "qtype"), cols)
tt<- dt_93[, .SD, .SDcols=cols ] 

x<- list( c(paste0(0:40, "_relation")), c(paste0(0:40, "_gender")), c(paste0(0:40, "_age")), c(paste0(0:40,"_edu")), c(paste0(0:40, "_work")), c(paste0(0:40, "_marstat")), c(paste0(0:40, "_children")), c(paste0(0:40, "_livstat")), c(paste0(0:40, "_livwhere")), c(paste0(0:40, "_meet")), c(paste0(0:40, "_tele")),c(paste0(0:40, "_help_adl")),c(paste0(0:40, "_help_iadl")))

dt_93_reshape <- reshape(tt, varying =x , direction = "long", timevar = "ID_i", idvar = c("ser_no", "survey_year", "qtype") , v.names = c("relationship","gender","age","edu","work","marstat","children","livstat","livwhere","meet","tele", "help_adl", "help_iadl"))
